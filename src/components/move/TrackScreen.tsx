...
const STAGES: StageDef[] = [
  ...
  { title: "Heading to pickup", pattern: /head.*pickup|to pickup|on the way(?!.*drop)/i, estimateOffsetMin: 5 },
  { title: "Loading your items", pattern: /load/i, estimateOffsetMin: 20 },
  { title: "In transit", pattern: /transit|to drop|en route to dest/i, estimateOffsetMin: 45 },
  { title: "Arrived at destination", pattern: /arriv|at destination|at drop/i, estimateOffsetMin: 65 },
  { title: "Move complete", pattern: /complete|delivered/i, estimateOffsetMin: 100 },
];
...
export function TrackScreen({ onRate, onHistory, onWallet }: { onRate: () => void; onHistory: () => void; onWallet?: () => void }) {
  const { user } = useAuth();
  ...
  const headline = isCompleted
    ? "Move complete"
    : currentIndex >= 1 && toPickupMinutes != null && currentIndex < 3
      ? `Arriving in ${toPickupMinutes} min`
      : currentIndex >= 3 && toDropoffMinutes != null
        ? `Arriving in ${toDropoffMinutes} min`
  ...
  if (!booking) {
    return (
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", background: "#F5F4EF" }}>
        <p style={{ font: "600 15px 'Hanken Grotesk'", color: "#6B6B70" }}>Loading your move…</p>
      </div>
    );
  }
      ...
      left={
        <div style={{ flex: 1, overflow: "auto", padding: "26px 28px 22px" }}>
          ...
          <button
            ...
            style={{ marginTop: 20, height: 44, width: "100%", borderRadius: 12, border: "1.5px solid rgba(0,0,0,.14)", background: "#fff", font: "700 13px 'Hanken Grotesk'", cursor: "pointer" }}
          >
            Manage move
          </button>
        </div>
      ...
      right={
        <>
          {mapHasRoute ? (
            ...
            <div style={{ height: "100%", display: "flex", alignItems: "center", justifyContent: "center", font: "600 14px 'Hanken Grotesk'", color: "#6B6B70" }}>
              Map loads when pickup &amp; dropoff are set
            </div>
          )}
          {!isCompleted && (currentIndex < 3 ? toPickupMinutes : toDropoffMinutes) != null && (
            <MapPill position="top-left">Arriving in {currentIndex < 3 ? toPickupMinutes : toDropoffMinutes} min</MapPill>
          )}
          <div style={{ position: "absolute", bottom: 24, left: 24, background: "#fff", borderRadius: 14, padding: "12px 16px", font: "600 13px 'Hanken Grotesk'", boxShadow: "0 10px 26px rgba(0,0,0,.14)", zIndex: 2 }}>
            {moverLabel} · <b style={{ font: "800 13px 'Archivo'" }}>live GPS</b>
          </div>
        </>
      }
    >
