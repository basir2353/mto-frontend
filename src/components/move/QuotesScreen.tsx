...
export function QuotesScreen({
  ...
  useEffect(() => {
    ...
    const poll = () => {
      void refreshRequest(request.id);
    };
    poll();
    const t = setInterval(poll, 8000);
    return () => clearInterval(t);
  }, [request?.id, quotes.length, timedOut, keepWaiting, refreshRequest]);
  ...
  useEffect(() => {
    ...
    // Intentionally ignore `quotes` identity — only react when the id set changes.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [quotes.map((q) => q.id).join(","), selectedQuoteId, onSelectQuote]);
    ...
    <WizardShell
      ...
      right={
        <>
          <RouteMap
            pickup={
              f.pickupPlace.address
                ? f.pickupPlace
                : { address: request?.pickupAddress ?? "" }
            }
            destination={
              f.destinationPlace.address
                ? f.destinationPlace
    ...
    return (
      <div className={styles.finding}>
        ...
        <p style={{ ...stepSub, marginBottom: 20 }}>
          Nearby movers were notified, but no quotes arrived in the last 10 minutes. Keep waiting or cancel this request.
        </p>
