...
const BookingMapOverlayContext = createContext<ReactNode>(null);

/** Puts Active/Draft (etc.) chips on every wizard map pane. */
export function BookingMapOverlayProvider({
  overlay,
  children,
}: {
  overlay: ReactNode;
  ...
  return (
    <BookingMapOverlayContext.Provider value={overlay}>{children}</BookingMapOverlayContext.Provider>
  );
}
...
export function WizardShell({
  ...
  const mapOverlay = useContext(BookingMapOverlayContext);
  ...
  return (
    <div
      className={`wizard-shell wizard-sheet-${mobileSheetSize} ${isCollapsed ? "wizard-sheet-collapsed" : "wizard-sheet-expanded"} ${desktopFormOpen ? "" : "wizard-form-hidden"}`}
    ...
    >
      <div
        ...
        style={{
          width: desktopFormOpen ? 420 : 0,
          flex: "none",
          background: "#fff",
          borderRight: desktopFormOpen ? "1px solid rgba(0,0,0,.08)" : "none",
          display: "flex",
      ...
      >
        ...
        <button
          type="button"
          className="wizard-desktop-collapse"
          aria-label="Hide booking panel"
          title="Hide panel"
      ...
      <div className="wizard-map-pane" style={{ flex: 1, position: "relative", overflow: "hidden", background: "#E7EAE3", minWidth: 0 }}>
        {!desktopFormOpen && (
          <button
            type="button"
            className="wizard-desktop-open"
            ...
            style={{
              ...
              alignItems: "center",
              justifyContent: "center",
              font: "800 20px var(--font-archivo, Archivo)",
      ...
      <style>{`
        @media(min-width:901px){
          .wizard-desktop-collapse{display:flex!important}
          .wizard-form-hidden .wizard-form-pane{width:0!important;border:0!important;pointer-events:none}
          .wizard-form-hidden .wizard-desktop-collapse{display:none!important}
        }
        @media(max-width:900px){
          .wizard-desktop-collapse,.wizard-desktop-open{display:none!important}
          ...
          .wizard-map-pane>div[style*="bottom: 24px"]:not(.route-metrics-badge){display:none!important}
          .wizard-map-pane>div[style*="top: 24px"]:not(.route-metrics-badge){top:76px!important;left:12px!important;right:auto!important;max-width:calc(100% - 24px)}
          .wizard-map-pane>div[style*="bottom: 24px"][style*="right: 24px"]:not(.route-metrics-badge){display:none!important}
          .route-metrics-badge{top:12px!important;left:16px!important;bottom:auto!important;right:auto!important}
...
export function RouteMetricsBadge({ pickup, destination }: { pickup: MapPlace; destination: MapPlace }) {
  ...
  return (
    ...
    >
      <div>
        <div style={{ font: "800 13px var(--font-archivo, Archivo)", lineHeight: 1.1 }}>{(tripKm * 0.621371).toFixed(1)} mi</div>
        <div style={{ font: "600 9px var(--font-hanken, 'Hanken Grotesk')", color: "rgba(255,255,255,.6)", letterSpacing: ".04em" }}>DISTANCE</div>
      </div>
      <div style={{ width: 1, alignSelf: "stretch", margin: "8px 0", background: "rgba(255,255,255,.15)" }} />
      <div>
        <div style={{ font: "800 13px var(--font-archivo, Archivo)", lineHeight: 1.1 }}>~{tripMinutes ?? "-"} min</div>
        <div style={{ font: "600 9px var(--font-hanken, 'Hanken Grotesk')", color: "rgba(255,255,255,.6)", letterSpacing: ".04em" }}>DRIVE</div>
      </div>
    </div>
  );
}
...
export function SoftNotice({ children }: { children?: ReactNode }) {
  ...
  return (
    <p
      ...
      style={{
        margin: "0 0 10px",
        font: "500 11px var(--font-hanken, 'Hanken Grotesk')",
        color: "#9A9AA0",
...
export function WizardFooter({
  ...
  busy?: boolean;
  disabled?: boolean;
  /** Quiet hint above actions — no banner styling. */
  notice?: string | null;
}) {
  return (
    <div>
      <SoftNotice>{notice}</SoftNotice>
